  ...
  return (
     <div className='w-[85%] flex flex-wrap mx-auto'>
        <div className='w-full'>
         ...
         items-center flex-col text-4xl text-slate-600
         font-bold relative pb-[45px]'>
            <h2>Kiemelt termékek</h2>
{/*Vonal elhelyezése a cím alá*/}
            <div className='w-[100px] h-[2px] bg-[#059473] mt-4'></div>
         </div>
        </div>
{/*Kiemelt termékek megjelenítése*/}
      <div className='w-full grid grid-cols-4 md-lg:grid-cols-3
      md:grid-cols-2 sm:grid-cols-1 gap-6'>
        {
          products.map((p,i) => <div key={i} className='border
          group transition-all duration-500 hover:shadow-md
          hover:-mt-3'>
            <div className='relative overflow-hidden'>
  {/*Kiemelt termékeknél ha van kedvezmény, akkor írja ki, ha nincs, akkor ne írja */}
      {
        p.discount ? <div className='flex justify-center items-center absolute
        text-white w-[38px] h-[38px] rounded-full bg-red-500
        font-semibold text-xs left-2 top-2'>{p.discount}%</div> : ''
      }
           <img className='sm:w-full w-full h-[240px] object-contain'
           src={p.images[0]} alt="" />
{/*A kép alól ugorjanak be a további ikonok*/}
    ...
    <li onClick={() => add_wishlist(p)} className='w-[38px] h-[38px]
    cursor-pointer bg-white flex justify-center items-center 
    rounded-full hover:bg-[#059473] hover:text-white
    hover:rotate-[720deg] transition-all'>
      <FaRegHeart />
    </li>
    <Link to={`/product/details/${p.slug}`} className='w-[38px] h-[38px]
    cursor-pointer bg-white flex justify-center items-center
    rounded-full hover:bg-[#059473] hover:text-white
    hover:rotate-[720deg] transition-all'>
      <FaEye />
