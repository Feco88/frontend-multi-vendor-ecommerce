...
import { customer_review, get_reviews, messageClear, product_details } from
'../store/reducers/homeReducer';
...
//vélemény elküldése a termékadathoz
const review_submit = (e) => {
//az oldal  nem frissül a vélemény elküldésekor
 e.preventDefault()
 const obj = {
  name: userInfo.name,
  review: re,
...
useEffect(() => {
 ...
 return (
    <div className='mt-8'>
{/*Összesített értékelések rész*/}
  <div className='flex gap-10 md-lg:flex-col'>
   <div className='flex flex-col gap-2 jusitfy-start
    items-start py-4'>
     <div>
      <span className='text-6xl font-semibold'>{product.rating}</span>
      <span className='text-3xl font-semibold
      text-slate-600'>/5</span>
     </div>
     <div className='flex text-3xl'>
     <Rating ratings={product.rating} />
     </div>
      <p className='text-sm text-slate-600'>({totalReview}) Értékelés alapján</p>
     </div>
   ...
   <div className='flex justify-start items-center gap-5'>
    ...
    <div className='flex justify-start items-center gap-5'>
     ...
     <div className='w-[200px] h-[14px] bg-slate-200 relative'>
     <div style={{width: `${Math.floor(( 100 * (rating_review[2]?.sum || 0)) / totalReview)}%`}}
     className='h-full bg-[#Edbb0E] w-[40%]'>
      ...
      <div className='flex justify-start items-center gap-5'>
        ...
        <div className='w-[200px] h-[14px] bg-slate-200 relative'>
         <div style={{width: `${Math.floor(( 100 * (rating_review[3]?.sum || 0)) / totalReview)}%`}}
      ...
      <div className='flex justify-start items-center gap-5'>
        ...
        <div className='w-[200px] h-[14px] bg-slate-200 relative'>
         <div style={{width: `${Math.floor(( 100 * (rating_review[4]?.sum || 0)) / totalReview)}%`}}
...
{/*Termék megtekintések száma, hozzászólások rész */}
 <h2 className='text-slate-600 text-xl font-bold py-5'>({totalReview}) értékelés</h2>
...
{/*Lapozás - Pagination */}
  <div className='flex justify-end'>
   ...
   totalReview > 5 && <Pagination pageNumber={pageNumber}
    ...
    parPage={parPage} showItem={Math.floor(totalReview / 3)}/>
   }
  </div>
    </div>
{/*Hozzászólás írása, csak akkor, amikor a felhasználó be van jelentkezve */}
    <div>
        {
...
{/*Hozzászólás bemeneti mező */}
  <form onSubmit={review_submit}>
   <textarea value={re} onChange={(e)=>setRe(e.target.value)} required
   className='border outline-0 p-3 w-full resize-none'
   name="" id="" cols="30" rows="5"></textarea>
  <div className='my-4'>
    <button className='py-1 px-5 hover:shadow-lg
    hover:shadow-red-500/40 bg-red-500 text-white rounded-sm'>
      Értékelés beküldése</button>
  </div>
  </form>
    </div> : <div>
      <Link to='/login' className='py-1 px-5 bg-red-500
      text-white ruonded-sm'>
      A hozzászóláshoz jelentkezz be</Link>
    </div>
  }
  </div>
 </div>
